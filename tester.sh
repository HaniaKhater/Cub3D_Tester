...
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/id_missing.cub

echo -e "${RED}--> Has a type identifier multiple times${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/id_multiple.cub

echo -e "${RED}--> A type identifier is not the first element of a line${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/id_not_first.cub

echo -e "${RED}--> A texture is not readable${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/nonvalid_texture.cub

echo -e "${RED}--> Color has an extra comma${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/extra_comma.cub

echo -e "${RED}--> Color has an rgb higher than 255${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/300rgb.cub

echo -e "${GREEN}--> Has spaces befor type identifier${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/map.cub
...
valgrind ./cub3D Cub3D_Tester/maps/nonmap_char.cub

echo -e "${RED}--> Two players${RESET}"
valgrind ./cub3D Cub3D_Tester/maps/two_players.cub

echo -e "${RED}--> No players${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/no_players.cub

echo -e "${RED}--> Not surrounded by walls${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/not_surrounded.cub

echo -e "${RED}--> has spaces in the middle of the map${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/spaced.cub

echo -e "${GREEN}--> Egypt${RESET}"
valgrind --leak-check=full --show-leak-kinds=all ./cub3D Cub3D_Tester/maps/Maze.cub
